...
//const float REACH_GOAL = 100;
//const float EFFICIENCY = 15;

const float REACH_GOAL = 1.2;
const float EFFICIENCY = 0.1;
const float LANE_DIFF = 0.2;
...
float inefficiency_cost(const Vehicle & vehicle, const vector<Vehicle> & trajectory, const map<int, vector<Vehicle>> & predictions, map<string, float> & data) {
    ...
    */
    ...
    cout <<"INEFF cost: "<<cost<<endl;
    return cost;
}


//if target lane is far away from goal lane penalize it.
...
map<string, float> get_helper_data(const Vehicle & vehicle, const vector<Vehicle> & trajectory, const map<int, vector<Vehicle>> & predictions) {
    ...
    */
    ...
    //in prep_lane_change_trajectory, lane of the successor state is not updated,
    //hence intended lane needs to be +1 (PLCL) -1 (PLCR),
    //final lane is successor state lane(=current state lane)
    //
    //in lane_change_trajectory lane of the successor state is updated
    //intended and final lane are the same (lane in successor trajectory)
    //
    //in keep_lane_trajectory intended lane is same as successor state lane=current state lane
    //
